...
router.get('/:id?',
 function(request, response) {
  ...
  if (request.params.id) {
      //borrow_modelin getAllId rivi 4
...
//kutsutaa borrowmodelin addiin rivi 10
router.post('/', 
function(request, response) {
...
router.put('/:id', 
function(request, response) {
  borrow.update(request.params.id, request.body, function(err, dbResult) { // jos ei vastaa err muuten dbresultti
    if (err) {
      response.json(err);
    } else {
      response.json(dbResult);
