...
const borrow = {
  getById: function(id, callback) {
    return db.query('select DATE_FORMAT(borrow_date,"%d.%m.%Y"),DATE_FORMAT(return_date,"%d.%m.%Y"),id_book, id_customer from borrow where id_borrow=?', [id], callback);
  },
  getAll: function(callback) {
    return db.query('select DATE_FORMAT(borrow_date,"%d.%m.%Y"),DATE_FORMAT(return_date,"%d.%m.%Y") from borrow', callback);
  },
